...
def train_one_event(
    event_id, catalog_df, labels_df, features_df, matches_df, splits
):
    ...
    val_metrics = evaluate(model, X_val, y_val, train_base_rate) if len(y_val) > 0 else {}
    test_metrics = evaluate(model, X_test, y_test, train_base_rate) if len(y_test) > 0 else {}
...
for _, ev in catalog.iterrows():
    ...
    print(f"Training {event_id} ...")
    result = train_one_event(event_id, catalog, labels, features, matches, splits)
    model = result["model"]
